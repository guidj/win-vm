...
# Graphic card passthrough (GK104-NVIDIA 670GTX)
...
# OVMF
cp /usr/share/edk2.git/ovmf-x64/OVMF_VARS-pure-efi.fd $BDIR/lib/my_vars.fd
OPTS="$OPTS -drive if=pflash,format=raw,readonly,file=/usr/share/edk2.git/ovmf-x64/OVMF_CODE-pure-efi.fd"
OPTS="$OPTS -drive if=pflash,format=raw,file=$BDIR/lib/my_vars.fd"
 
 
# System drive
OPTS="$OPTS -drive id=disk0,if=none,cache=unsafe,format=raw,file=$BDIR/lib/windows10.img"
...
OPTS="$OPTS -drive id=cd0,if=none,format=raw,readonly,file=$BDIR/pkg/Win10_1607_English_x64.iso"
OPTS="$OPTS -device driver=ide-cd,bus=ide.0,drive=cd0"
 
 
# Virtio driver
OPTS="$OPTS -drive id=virtiocd,if=none,format=raw,file=$BDIR/pkg/virtio-win-latest.iso"
